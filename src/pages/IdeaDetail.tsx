  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-3 h-8 border-2 border-[var(--color-border-strong)] text-[12px] font-bold uppercase tracking-[0.08em] hover:border-[var(--color-green)] transition-colors"
      >
        Status ändern
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 sm:right-0 sm:left-auto top-full mt-1 w-48 bg-[var(--color-surface-1)] border-2 border-[var(--color-border-strong)] z-[100] py-1" style={{ boxShadow: "var(--shadow-brutal)" }}>
            {STATUS_ORDER.map((s) => (
              <button
                key={s}
                onClick={() => { onChange(s); setOpen(false); }}
                className={`w-full text-left px-3 py-1.5 text-[12px] font-bold uppercase tracking-[0.06em] hover:bg-[var(--color-surface-2)] hover:text-[var(--color-green)] transition-colors flex items-center gap-2 ${
                  s === current ? "text-[var(--color-green)]" : "text-[var(--color-text-secondary)]"
                }`}
              >
...
export function ScriptEditor({ ideaId, ideaTitle, ideaDescription, clientName, clientCompany, clientContext, clientPlatforms, clientMainPlatform }: { ideaId: string; ideaTitle: string; ideaDescription?: string; clientName: string; clientCompany?: string; clientContext?: string; clientPlatforms?: string[]; clientMainPlatform?: string }) {
  ...
  return (
    <div className="bg-[var(--color-surface-1)] border-2 border-[var(--color-border-strong)] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b-2 border-[var(--color-border-strong)]">
        <div className="flex items-center gap-3">
          ...
          {latestScript && (
            <span className="text-[11px] font-mono text-[var(--color-green)] border border-[var(--color-green)] px-1.5 py-0.5">
        ...
        <div className="flex items-center gap-1.5">
          {user?.role === "admin" && !editing && (
            <button
              ...
              className="flex items-center gap-1.5 h-7 px-3 border-2 border-[var(--color-border-strong)] text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-secondary)] hover:border-[var(--color-green)] hover:text-[var(--color-green)] transition-colors"
            >
      ...
      {editing ? (
        <div className="p-4">
          ...
          {/* AI Buttons */}
          {user?.role === "admin" && (
            <div className="flex items-center gap-0 mt-3 pb-3 border-b-2 border-[var(--color-border-strong)]">
              ...
              <button
                ...
                disabled={aiLoading}
                className="flex items-center gap-1.5 h-7 px-3 text-[12px] font-bold uppercase tracking-[0.06em] disabled:opacity-50 transition-all -mr-[2px]"
                style={{ background: "linear-gradient(to right, rgba(139,92,246,0.1), rgba(99,102,241,0.1))", color: "var(--color-violet-light)", borderWidth: 2, borderColor: "rgba(139,92,246,0.3)" }}
              >
              ...
              {content.trim() && (
                <>
                  <button
                    ...
                    disabled={aiLoading}
                    className="flex items-center gap-1.5 h-7 px-3 text-[12px] font-bold uppercase tracking-[0.06em] disabled:opacity-50 transition-all -mr-[2px]"
                    style={{ background: "linear-gradient(to right, rgba(245,158,11,0.1), rgba(249,115,22,0.1))", color: "#FCD34D", borderWidth: 2, borderColor: "rgba(245,158,11,0.3)" }}
                  >
                  ...
                  <button
                    ...
                    disabled={aiLoading}
                    className="flex items-center gap-1.5 h-7 px-3 text-[12px] font-bold uppercase tracking-[0.06em] disabled:opacity-50 transition-all"
                    style={{ background: "linear-gradient(to right, rgba(16,185,129,0.1), rgba(20,184,166,0.1))", color: "#6EE7B7", borderWidth: 2, borderColor: "rgba(16,185,129,0.3)" }}
                  >
          ...
          <div className="flex justify-end gap-0 mt-3">
            <button
              ...
              className="h-8 px-4 border-2 border-[var(--color-border-strong)] text-[12px] font-bold uppercase tracking-[0.06em] hover:border-[var(--color-green)] transition-colors -mr-[2px]"
            >
              Abbrechen
            </button>
            <button
              ...
              className="flex items-center gap-1.5 h-8 px-4 bg-[var(--color-green)] text-[var(--color-surface-0)] text-[12px] font-bold uppercase tracking-[0.08em] border-2 border-[var(--color-green)] hover:-translate-x-0.5 hover:-translate-y-0.5 disabled:opacity-50 transition-all"
            >
        ...
        <div className="px-4 py-8 text-center">
          <div className="w-10 h-10 border-2 border-[var(--color-border-strong)] flex items-center justify-center mx-auto mb-3">
          ...
          {user?.role === "admin" && (
            <button
              onClick={() => handleAI("generate")}
              disabled={aiLoading}
              className="mt-3 inline-flex items-center gap-1.5 h-8 px-4 text-[13px] font-bold uppercase tracking-[0.06em] disabled:opacity-50 transition-all"
              style={{ background: "linear-gradient(to right, rgba(139,92,246,0.1), rgba(99,102,241,0.1))", color: "var(--color-violet-light)", border: "2px solid rgba(139,92,246,0.3)" }}
            >
    ...
    <div>
      ...
      <div className="flex flex-col mb-4">
        {pendingComments.map((pc) => (
          <div
            key={pc.id}
            className="border-2 border-[var(--color-green)] bg-[var(--color-surface-0)] p-3 animate-pulse -mb-[2px]"
          >
        ...
        {(comments || []).filter(c => !c.parentId).map((comment) => (
          <div
            ...
            className={`border-2 p-3 transition-colors -mb-[2px] last:mb-0 ${
              comment.resolved
                ? "border-[var(--color-border-strong)] bg-[var(--color-surface-0)] opacity-60"
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              {!comment.resolved && user?.role === "admin" && (
                <button
                  ...
                  className="p-1 border-2 border-transparent text-[var(--color-text-tertiary)] transition-colors flex-shrink-0"
                  style={{ ["--hover-border" as string]: "var(--color-success)" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--color-success)"; (e.currentTarget as HTMLElement).style.color = "var(--color-success)"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "transparent"; (e.currentTarget as HTMLElement).style.color = "var(--color-text-tertiary)"; }}
                  title="Erledigt"
                >
      ...
      <form onSubmit={handleSubmit} className="flex gap-0">
        <input
          ...
          className="flex-1 h-9 px-3 border-2 border-[var(--color-border-strong)] bg-[var(--color-surface-1)] text-[14px] focus:border-[var(--color-green)] focus:outline-none transition-colors -mr-[2px]"
          placeholder="Kommentar schreiben…"
        />
        <button
          type="submit"
          disabled={!newComment.trim()}
          className="h-9 w-9 bg-[var(--color-green)] text-[var(--color-surface-0)] flex items-center justify-center border-2 border-[var(--color-green)] hover:-translate-x-0.5 hover:-translate-y-0.5 disabled:opacity-30 transition-all"
        >
          ...
          {(videos || []).map((video) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 p-3 border-2 border-[var(--color-border-strong)] bg-[var(--color-surface-1)] hover:border-[var(--color-green)] hover:shadow-brutal hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all text-left group -mb-[2px] last:mb-0"
              >
                <div className="relative w-20 h-12 overflow-hidden bg-black flex-shrink-0 border border-[var(--color-border-strong)]">
                  {thumb ? (
                    <img src={thumb} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <Film className="w-5 h-5 text-[var(--color-text-muted)]" />
                    </div>
                  )}
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30">
                    <div className="w-8 h-8 bg-[var(--color-green)] flex items-center justify-center">
                      <Play className="w-4 h-4 text-[var(--color-surface-0)] ml-0.5" />
                    </div>
                  </div>
                </div>
...
export function IdeaDetail({ ideaId, onBack, onNavigate }: { ideaId: string; onBack: () => void; onNavigate?: (page: string, id?: string) => void }) {
  ...
  if (!idea) {
    ...
    return (
      <div className="p-8">
        <button onClick={onBack} className="flex items-center gap-1.5 text-[13px] font-bold uppercase tracking-[0.06em] text-[var(--color-text-secondary)] hover:text-[var(--color-green)] transition-colors mb-4 border-2 border-transparent hover:border-[var(--color-green)] px-2 py-1">
    ...
    <div className="max-w-[720px] mx-auto px-6 lg:px-8 py-6">
      ...
      <nav className="flex items-center gap-1 text-[11px] font-bold uppercase tracking-[0.1em] mb-6 animate-in">
        <button
          ...
          className="text-[var(--color-text-tertiary)] hover:text-[var(--color-green)] transition-colors"
        >
          Dashboard
        </button>
      ...
      {/* Description */}
      {idea.description && (
        <div className="animate-in stagger-1 bg-[var(--color-surface-1)] border-2 border-[var(--color-border-strong)] p-4 mb-6">
      ...
      {user?.role === "admin" && (
        <div className="animate-in stagger-2 flex items-center gap-3 mb-6 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-0">
            <div className="flex items-center gap-2 h-7 px-2 border-2 border-[var(--color-border-strong)] -mr-[2px]">
            ...
            <button
              ...
              className="h-7 px-2.5 border-2 border-[var(--color-border-strong)] text-[11px] font-bold uppercase tracking-[0.06em] hover:border-[var(--color-green)] transition-colors flex items-center gap-1 text-[var(--color-text-tertiary)]"
        ...
        <div className="animate-in stagger-2 mb-6">
          <div className="flex flex-col">
            {ideaShootDates.map((sd) => (
              <div key={sd._id} className="flex items-center gap-3 bg-[var(--color-surface-1)] border-2 border-[var(--color-border-strong)] px-4 py-3 -mb-[2px] last:mb-0">
                <div className="w-8 h-8 border-2 border-[var(--color-border-strong)] flex items-center justify-center flex-shrink-0">
